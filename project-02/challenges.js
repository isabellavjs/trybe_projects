...
console.log(verifyRepeatedNumbers([1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 15]))

function completePhoneNumber(arrayPhoneNumber) {
  let generatedNumber = '';
...
console.log(completePhoneNumber([1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 1]));

function generatePhoneNumber(arrayPhoneNumber) {
   if (arrayPhoneNumber.length !== 11) {
     return 'Array com tamanho incorreto.';
   } 
   let repeatedNumbers = verifyRepeatedNumbers(arrayPhoneNumber);
   for (let index = 0; index < arrayPhoneNumber.length; index += 1) {
     if (arrayPhoneNumber[index] < 0 || arrayPhoneNumber[index] > 9 || repeatedNumbers) {
       return 'não é possível gerar um número de telefone com esses valores';
     }
   }
   return completePhoneNumber(arrayPhoneNumber);
  }
